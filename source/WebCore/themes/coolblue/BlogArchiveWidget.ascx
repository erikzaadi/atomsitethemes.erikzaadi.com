    ...
    <ul>
     <% foreach (int year in Model.GetYears()) { %>
		    <% foreach (int month in Model.GetMonths(year)) { %>
			    <li><a href="<%= Url.RouteIdUrl("BlogDateMonth", Model.Feed.Id, new { year = year, month = month.ToString("00") }) %>"><%= new DateTime(year, month, 1).ToString("MMMM yyyy")%> (<%= Model.GetEntries(year, month).Count() %>)</a></li>
     <% } } %>
    </ul>
    <% } else { %>
