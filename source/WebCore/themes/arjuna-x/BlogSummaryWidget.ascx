...
<h4>
    ...
    <ol class="entries">
        ...
        <li>
            <h4>
                <strong><a href="<%= Url.RouteIdUrl("BlogEntry", entry.Id) %>">
                    <%= entry.Title.Text %></a></strong> by
            ...
            <div class="filedUnder">
                Filed under:
                <% Html.RenderPartial("BlogCategories", new CategoriesModel() { Categories = entry.Categories, Id = Model.Feed.Id }); %>
            </div>
            <% } %>
            <em>Posted
                <%= Html.DateTimeAbbreviation(entry.Date) %></em> </li>
        <%} %>
    </ol>
    <%}
       else
