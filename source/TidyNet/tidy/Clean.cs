...
namespace TidyNet
{
	...
	internal class Clean
	{
		...
		*/
        ...
        if (lexer.Options.CombineNestedDivs)
        {
		...
		/* replace strike by del */
		public virtual void DelFromStrike(Node node)
		{
			while (node != null)
			{
				if (node.Tag == _tt.TagStrike)
				{
					node.Element = _tt.TagDel.Name;
					node.Tag = _tt.TagDel;
				}
				
				if (node.Content != null)
				{
					DelFromStrike(node.Content);
				}
