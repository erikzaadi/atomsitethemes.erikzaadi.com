...
namespace TidyNet
{
	...
	internal class EntityTable
	{
		...
		public virtual Entity Lookup(string name)
		{
			return (Entity)_entityHashtable[name];
		}
		
		public virtual Entity Install(string name, short code)
		{
			Entity ent = Lookup(name);
			if (ent == null)
			{
				ent = new Entity(name, code);
				_entityHashtable[name] = ent;
			}
			else
			{
				ent.Code = code;
		...
		public virtual Entity Install(Entity ent)
		{
			object entity = _entityHashtable[ent.Name];
			_entityHashtable[ent.Name] = ent;
			return (Entity)entity;
		}
		
		/* entity starting with "&" returns zero on error */
		public virtual short EntityCode(string name)
		{
			int c;
			...
			if (name[1] == '#')
			{
				...
				try
				{
					if (name.Length >= 4 && name[2] == 'x')
					{
						c = Convert.ToInt32(name.Substring(3), 16);
					}
					else if (name.Length >= 3)
					{
						c = Int32.Parse(name.Substring(2));
					}
				}
		...
		public virtual string EntityName(short code)
		{
			string name = null;

			foreach (Entity ent in _entityHashtable.Values)
			{
				if (ent.Code == code)
				{
					name = ent.Name;
		...
		public static EntityTable DefaultEntityTable
		{
			get
			{
				if (_defaultEntityTable == null)
				{
					_defaultEntityTable = new EntityTable();
					for (int i = 0; i < _entities.Length; i++)
					{
						_defaultEntityTable.Install(_entities[i]);
					}
				}
		...
		private static Entity[] _entities = new Entity[]
			{
				new Entity("nbsp", 160),
