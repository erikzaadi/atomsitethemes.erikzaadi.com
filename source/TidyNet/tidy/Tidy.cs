	...
	public class Tidy
	{
		...
		internal Node ParseInternal(Stream input, string file, Stream Output, TidyMessageCollection messages)
		{
			...
			if (input != null)
			{
              ...
              {
                cleaner.DelFromStrike(document);
                cleaner.UnderlineToSpan(document);
                cleaner.CleanTableAttrs(document);
                cleaner.CleanImgAttrs(document);
                cleaner.CleanAnchorTarget(document);
              }
						}
						else
						{
